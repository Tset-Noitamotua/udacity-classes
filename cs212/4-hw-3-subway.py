...
def subway(**lines):
    ...
    for line, t in lines.items():
        ...
        for i in range(len(stations)):
            if stations[i] not in result: result[stations[i]] = {}
            if i > 0: result[stations[i]][stations[i-1]] = line
            if i < (len(stations) - 1): result[stations[i]][stations[i+1]] = line
    return result
...
def longest_ride(system):
    ...
    stops = set(s for s in system.keys())
...
def test_ride():
    ...
    print 'longest ride 1 passed!'
    assert len(path_states(longest_ride(boston))) == 16
    print 'longest ride 2 passed!'
