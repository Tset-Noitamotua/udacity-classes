...
def post_order(S, root):
    ...
    while len(q) > 0:
        ...
        children = False
        for t in S[n]:
            if S[n][t] == 'green' and t not in seen:
                q.append(t)
                seen[t] = True
                children = True
        if not children:
...
def test_post_order():
    ...
    #print {'a':7, 'b':1, 'c':6, 'd':5, 'e':4, 'f':2, 'g':3}
...
def number_of_descendants(S, root):
    ...
    while len(q) > 0:
        ...
        children = False
        for t in S[n]:
            if t not in seen and S[n][t] == 'green':
                q.append(t)
                seen[t] = True
                children = True
        if not children:
            ...
            q.pop(len(q)-1)
            for t in S[n]:
                if S[n][t] == 'green' and t in mapping:
                    mapping[n] += mapping[t]
...
def test_number_of_descendants():
    ...
    #print {'a':7, 'b':1, 'c':5, 'd':4, 'e':3, 'f':1, 'g':1}
