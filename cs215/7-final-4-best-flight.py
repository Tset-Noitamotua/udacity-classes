...
def connect_to_path(FINFO, paths, flight):
    ...
    for i in range(len(paths)):
        ...
        if timevalue(fi1['arrv']) < timevalue(fi2['dept']) and assert_no_loops(FINFO, paths[i], fi2['dest']):
...
def print_route(FINFO, path):
    print path, ':'
    for f in path:
        print '%s to %s, %s - %s' % (FINFO[f]['orig'], FINFO[f]['dest'], FINFO[f]['dept'], FINFO[f]['arrv'])
    print '---'
    print evaluate_path(FINFO, path)

def create_weighted_graph(flights):
    G = {}
    FINFO = defaultdict(dict)
    
    for (fn, orig, dest, dept, arrv, cost) in flights:
        if orig not in G:
            G[orig] = {}
        if dest not in G[orig]:
            G[orig][dest] = []
        tdept = timevalue(dept)
        tarrv = timevalue(arrv)
        duration = timediff(tdept, tarrv)
        info = (cost, duration, fn)
        G[orig][dest].append(info)
        FINFO[fn]['orig'] = orig
        FINFO[fn]['dest'] = dest
        FINFO[fn]['dept'] = dept
        FINFO[fn]['arrv'] = arrv
...
def test():
    ...
    flights = find_best_flights(all_flights, 'Perth', 'Laverton')
    print flights
    assert flights == [321]
