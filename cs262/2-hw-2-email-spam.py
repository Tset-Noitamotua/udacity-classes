...
def addresses(haystack): 
  addresses = re.findall(r'(?:[a-zA-Z0-9.-_]+)@(?:[a-zA-Z]+\.)+[A-Za-z]+', haystack)
  addresses = map(lambda x: x.replace('NOSPAM', ''), addresses)
  return addresses
...
deutsche bundespost stamp. seti@home is not actually an email address. Then
this one NOSPAMuser@uucpgateway.somedomain.example.com test tester testing"""
