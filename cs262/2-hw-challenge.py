...
def nfsmtrim(edges, accepting):
    ...
    while len(queue) > 0:
        ...
        if t not in visited:
            visited.append(t)
        for edge, target in edges.iteritems():
            if t in target:
                if edge not in newedges:
                    newedges[edge] = [t]
                else:
                    newedges[edge].append(t)
                if edge[0] != t and edge[0] not in visited:
                    queue.append(edge[0])
                f = True
        if t in accepting and t not in newaccepting and f:
            newaccepting.append(t)
