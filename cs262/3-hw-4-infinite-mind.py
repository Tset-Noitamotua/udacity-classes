...
def cfginfinite(grammar):
    ...
    def cangrow(current, visited, sz):
        if current in visited:
            return sz > 0
        newvisited = visited + [current]
        for R in [rule[1] for rule in grammar if rule[0] == current]:
            for s in R:
                if cangrow(s, newvisited, sz + len(R) - 1):
                    return True
        return False
                    
    for L in [rule[0] for rule in grammar]:
        if cangrow(L, [], 0):
            return True
    return False
