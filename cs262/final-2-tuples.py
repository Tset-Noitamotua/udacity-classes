...
#
# 4. An expression can be LBRACKET "a list of more than one comma-separated
...
def p_tuple(p):
    'exp : LPAREN targs RPAREN'
    p[0] = ('tuple', p[2])

def p_list(p):
    'exp : LBRACKET largs RBRACKET'
    p[0] = ('list', p[2])

def p_largs(p):
    'largs : exp COMMA largs'
    p[0] = [p[1]] + p[3]
