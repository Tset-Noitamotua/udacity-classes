...
def evaluate(ast):
    result = {}
    for t in [i for i in ast if i[1] == 'has']:
        result[t[0]] = t[2]
        ast.remove(t)
    ...
    while len(ast) > 0:
        changed = False
        for t1 in ast:
            for t2 in [i for i in ast if i[0] <> t1[0] and i[1] <> t1[1] and i[2] == t1[2] and i[3] == t1[3]]:
                if t1[1] == 'buy' and t2[1] == 'sell' and result[t1[0]] >= t2[3]:
                    result[t2[0]] += t1[3]
                    result[t1[0]] -= t2[3]
                    ast.remove(t1) # bought!
                    ast.remove(t2) # sold!
                    changed = True
                if t2[1] == 'buy' and t1[1] == 'sell' and result[t2[0]] >= t1[3]:
                    result[t1[0]] += t2[3]
                    result[t2[0]] -= t1[3]
                    ast.remove(t1) # sold!
                    ast.remove(t2) # bought!
                    changed = True
        if not changed:
            break
