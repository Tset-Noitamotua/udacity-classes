...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        b = tan(a) * d / self.length
        t = self.orientation
        result = robot(self.length)
        result.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise)
        if b < 0.001:
            result.x = self.x + d * cos(t)
            result.y = self.y + d * sin(t)
            result.orientation = (b + t) % (2 * pi)
        else:
            R = self.length / tan(a)
            cx = self.x - R * sin(t)
            cy = self.y + R * cos(t)
            result.x = cx + R * sin(b + t)
            result.y = cy - R * cos(b + t)
            result.orientation = (b + t) % (2 * pi)
