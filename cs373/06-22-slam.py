...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    dim = 2 * (N + num_landmarks) # because it'll contain x & y
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1
    Omega.value[1][1] = 1
    ...
    for k in range(len(data)):
        n = k * 2 # index of robot pose in matrix
        measurement = data[k][0]
        motion = data[k][1]

        for i in range(len(measurement)):
            m = 2 * (N + measurement[i][0]) # index of landmark in matrix
            for b in range(2):
        ...
        for b in range(2):
            Omega.value[n+b+0][n+b+2] += -1.0 / motion_noise
            Omega.value[n+b+2][n+b+0] += -1.0 / motion_noise
            
            Xi.value[n+b+0][0] += -motion[b] / motion_noise
