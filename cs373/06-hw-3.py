...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        rows = len(Omega.value)
        cols = len(Omega.value[0])

        rows_in = [0, 1]
        rows_in.extend(range(4, rows + 2))
        
        cols_in = [0, 1]
        cols_in.extend(range(4, cols + 2))

        Omega = Omega.expand(rows + 2,  cols + 2, rows_in, cols_in)
        Xi = Xi.expand(rows + 2,  1, rows_in, [0])
        ...
        rows_t = range(2, rows + 2)
        cols_t = range(2, cols + 2)

        B = Omega.take([0, 1], [0, 1])
        A = Omega.take([0, 1], cols_t)
        C = Xi.take([0, 1], [0])
        
        Omega_prime = Omega.take(rows_t, cols_t)
        Xi_prime = Xi.take(rows_t, [0])
