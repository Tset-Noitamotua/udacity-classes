...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    costs = [[9999 for c in range(len(road[0]))] for r in range(len(road))]
    ...
    cost_add = [0, # straight
                lane_change_cost, # left shift
                lane_change_cost] # right shift
    ...
    while len(queue) > 0:
        ...
        for i in range(len(move)):
            r = current[0] + move[i][0]
            c = current[1] + move[i][1]

            if r >= 0 and c >= 0 and r < len(road) and c < len(road[0]) and road[r][c] != 0:
                tcost = costs[current[0]][current[1]] + (1. / road[r][c]) + cost_add[i]
...
def solution_check(test, epsilon = 0.00001):
    ...
    for i in range(len(test[0])):
        ...
        true_cost = test[4][i]
        print 'Test case', i+1, 'your answer is', user_cost, 'correct:', true_cost
