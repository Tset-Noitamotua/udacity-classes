...
def find_collision(message):
    ...
    def swap_blocks(a, b, ca, cb):
        ea = xor_bits(a, ca)
        eb = xor_bits(b, cb)
        new_a = xor_bits(ea, cb)
        new_b = xor_bits(eb, ca)
        return new_a, new_b

    block_size, block_enc, key, ctr = hash_inputs()
    cipher = counter_mode(message, key, ctr, block_size, block_enc)

    a = get_block(message, 0, block_size)
    b = get_block(message, 1, block_size)
    ca = get_block(cipher, 0, block_size)
    cb = get_block(cipher, 1, block_size)
    new_a, new_b = swap_blocks(a, b, ca, cb)
    new_message[0:block_size] = new_a
    new_message[block_size:2*block_size] = new_b
...
def test():     
    ...
    for m in messages:
        ...
        if not check(m, new_message):
            print "Failed to find a collision for '%s'" % om
            return False
        else:
            print 'Test passed for', om
    return True
